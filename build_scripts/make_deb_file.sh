...
# binary pack control file is located in build/DEBIAN_control
...
cp src/build/SeminalRootAngleExtractor.desktop dist/ubuntu_setup/usr/share/applications


mkdir dist/ubuntu_setup/DEBIAN
cp -r build/DEBIAN_control dist/ubuntu_setup/DEBIAN/control
cp -r dist/SeminalRootAngleExtractor dist/ubuntu_setup/usr/local/bin/SeminalRootAngleExtractor

# We do not have icons yet.
#cp -r main/icons dist/ubuntu_setup/usr/local/bin/SeminalRootAngleExtractor/icons

dpkg-deb --build dist/ubuntu_setup
mv dist/ubuntu_setup.deb dist/SeminalRootAngleExtractor.deb
echo 'output debian package to dist/SeminalRootAngleExtractor.deb'
